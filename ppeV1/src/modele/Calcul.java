...
public class Calcul {
	...
	public static void combat(Figurine f1,Figurine f2,int z){
		...
		int blessure = 0;
		int degat_block = 0;
		for(int i=0 ;i<f1.getArmes().get(z).getA();i++ ){
			boolean jet1=false;   
			boolean jet2=false;
			boolean block=true;
			jet1=jet_de_touche(jet1,f1.getArmes().get(z).getC() );
			//apptitude relancer
				if(jet1==true){
					touche = touche + 1;
				...
				if(jet2==true){
					...
					if(block==false && f2.getHP() >0){
						f2.setHP(f2.getHP() - f1.getArmes().get(z).getD())  ;
						sum_degat = sum_degat +f1.getArmes().get(z).getD();
					}
					if(block == true) {
